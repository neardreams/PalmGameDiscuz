...
if($_GET['step'] == 'start') {
	...
	if($result == true) {
		$opensoso = '<br><br>友情提示：<br>為更好的降低論壇搜索時的數據壓力，本次升級已經幫本站開通縱橫搜索服務。<br>你可以在 <a href=\\\'../admin.php?frames=yes&action=cloud&operation=search\\\' target=\\\'_blank\\\'>站點後台-&gt;應用-&gt;縱橫搜索 進行管理</a>。';
	}
	...
	if($_GET['from']) {
		show_msg('<span id="finalmsg">緩存更新中，請稍候 ...</span><iframe src="../misc.php?mod=initsys" style="display:none;" onload="window.location.href=\''.$_GET['from'].'\'"></iframe>');
	} else {
		show_msg('<span id="finalmsg">緩存更新中，請稍候 ...</span><iframe src="../misc.php?mod=initsys" style="display:none;" onload="document.getElementById(\'finalmsg\').innerHTML = \'恭喜，數據庫結構升級完成！為了數據安全，請刪除本文件。'.$opensoso.'\'"></iframe>');
	}
