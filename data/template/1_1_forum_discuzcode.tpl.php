...
$return .= <<<EOF
，本帖隱藏的內容需要積分高於 {$creditsrequire} 才可瀏覽，您當前積分為 {$_G['member']['credits']}</div>
EOF;
?><?php 
return $return;
}
...
?><?php
$return = <<<EOF
<div class="locked">以下內容需要積分高於 {$creditsrequire} 才可瀏覽</div>
{$message}<br /><br />
...
?><?php
$return = <<<EOF
<div class="blockcode"><div id="{$randomid}"><ol><li>{$code}</ol></div><em onclick="copycode($('{$randomid}'));">複製代碼</em></div>
EOF;
?><?php 
return $return;
}
...
?><?php
$return = <<<EOF
<div class="showhide"><h4>本帖隱藏的內容</h4>\\1</div>
...
$return .= <<<EOF
，如果您要查看本帖隱藏內容請<a href="forum.php?mod=post&amp;action=reply&amp;fid={$_G['fid']}&amp;tid={$_G['tid']}" onclick="showWindow('reply', this.href)">回復</a></div>
EOF;
?><?php 
return $return;
}
...
<div class="tip_c">
<p class="y">{$attach['dateline']} 上傳</p>
<p>點擊文件名下載附件</p>

EOF;
 if(!$attach['attachimg'] && $_G['getattachcredits']) { 
$return .= <<<EOF
下載積分: {$_G['getattachcredits']}<br />
EOF;
 } 
$return .= <<<EOF
...
EOF;
 if($attach['readperm']) { 
$return .= <<<EOF
, 閱讀權限: <strong>{$attach['readperm']}</strong>
EOF;
 } 
$return .= <<<EOF
, 下載次數: {$attach['downloads']}
EOF;
 if(!$attach['attachimg'] && $_G['getattachcredits']) { 
$return .= <<<EOF
, 下載積分: {$_G['getattachcredits']}
EOF;
 } 
$return .= <<<EOF
</p>
...
售價: <strong>{$attach['price']} {$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit']}{$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title']}</strong> &nbsp;[<a href="forum.php?mod=misc&amp;action=viewattachpayments&amp;aid={$attach['aid']}" onclick="showWindow('attachpay', this.href)" target="_blank">記錄</a>]

EOF;
 if(!$attach['payed']) { 
$return .= <<<EOF

&nbsp;[<a href="forum.php?mod=misc&amp;action=attachpay&amp;aid={$attach['aid']}&amp;tid={$attach['tid']}" onclick="showWindow('attachpay', this.href)">購買</a>]
...
<em class="xg1">({$attach['attachsize']}, 下載次數: {$attach['downloads']})</em>
</p>
<div class="tip tip_4" id="aid{$attach['aid']}_menu" style="display: none" disautofocus="true">
<div>
<p>
<a href="forum.php?mod=attachment{$is_archive}&amp;aid={$aidencode}&amp;nothumb=yes" target="_blank">下載附件</a>

EOF;
 if(helper_access::check_module('album')) { 
$return .= <<<EOF

&nbsp;<a href="javascript:;" onclick="showWindow(this.id, this.getAttribute('url'), 'get', 0);" id="savephoto_{$attach['aid']}" url="home.php?mod=spacecp&amp;ac=album&amp;op=saveforumphoto&amp;aid={$attach['aid']}&amp;handlekey=savephoto_{$attach['aid']}">保存到相冊</a>

EOF;
 } if($firstpost && $_G['fid'] && $_G['forum']['picstyle'] && ($_G['forum']['ismoderator'] || $_G['uid'] == $attach['uid'])) { 
$return .= <<<EOF

&nbsp;<a href="forum.php?mod=ajax&amp;action=setthreadcover&amp;aid={$attach['aid']}&amp;fid={$_G['fid']}" onclick="showWindow('setcover{$attach['aid']}', this.href)">設為封面</a>
...
<p>
<span class="y">{$attach['dateline']} 上傳</span>
...
&nbsp;[<a href="forum.php?mod=misc&amp;action=attachpay&amp;aid={$attach['aid']}&amp;tid={$attach['tid']}" onclick="showWindow('attachpay', this.href)" target="_blank">購買</a>]
...
<div class="xs0">
<p><strong>{$attach['filename']}</strong> <em class="xg1">({$attach['attachsize']}, 下載次數: {$attach['downloads']})</em></p>
<p>
<a href="forum.php?mod=attachment{$is_archive}&amp;aid={$aidencode}&amp;nothumb=yes" target="_blank">下載附件</a>
...
<p class="xg1 y">{$attach['dateline']} 上傳</p>
...
<em class="xg1">({$attach['attachsize']}, 下載次數: {$attach['downloads']}
EOF;
 if($attach['price']) { 
$return .= <<<EOF
, 售價: {$attach['price']} {$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit']}{$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title']}
EOF;
 } 
$return .= <<<EOF
)</em>
...
<div class="tip_c xs0">
<div class="y">{$attach['dateline']} 上傳</div>
點擊文件名下載附件
...
$return .= <<<EOF
<br />閱讀權限: {$attach['readperm']}
EOF;
 } } if(!IS_ROBOT && !$guestviewthumb) { if($attach['price']) { 
$return .= <<<EOF

<br />售價: {$attach['price']} {$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit']}{$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title']}	&nbsp;<a href="forum.php?mod=misc&amp;action=viewattachpayments&amp;aid={$attach['aid']}" onclick="showWindow('attachpay', this.href)" target="_blank">[記錄]</a>
